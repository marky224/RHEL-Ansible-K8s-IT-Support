...
SCRIPTS_DIR="/usr/local/bin"  # Where Python scripts will live
...
if ! command -v ansible >/dev/null 2>&1; then
    ...
    dnf install -y python3 curl openssl || {
        log "Error: Failed to install python3, curl, and openssl"
        exit 1
    }
...
# Install Python scripts for HTTPS services
log "Installing check-in listener and SSH key server..."
for SCRIPT in "checkin_listener.py" "ssh_key_server.py"; do
    if [ ! -f "$SCRIPTS_DIR/$SCRIPT" ]; then
        curl -o "$SCRIPTS_DIR/$SCRIPT" "https://raw.githubusercontent.com/marky224/rhel-ansible-k8s-it-support/main/scripts/$SCRIPT" || {
...
log "HTTPS service scripts installed in $SCRIPTS_DIR"

# Create systemd service files for Python scripts
log "Configuring systemd services for HTTPS endpoints..."
cat > /etc/systemd/system/checkin-listener.service <<EOF
[Unit]
Description=Ansible Check-in Listener
After=network.target

[Service]
ExecStart=$SCRIPTS_DIR/checkin_listener.py --port $CHECKIN_PORT --cert $CA_CERT --key $CA_KEY
Restart=always
User=root
...
[Unit]
Description=Ansible SSH Key Server
After=network.target

[Service]
ExecStart=$SCRIPTS_DIR/ssh_key_server.py --port $CHECKIN_PORT --cert $CA_CERT --key $CA_KEY
Restart=always
User=root
...
for SERVICE in checkin-listener ssh-key-server; do
...
log "HTTPS services configured and running via systemd"

# Configure firewall
log "Configuring firewall for SSH and port $CHECKIN_PORT..."
